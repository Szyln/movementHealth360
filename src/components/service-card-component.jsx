...
function ServiceCardComponent({ service, getServiceName, isOpen = false }) {
  ...
  return (
    <div className="border-primary border-around rounded h-100p">
      {/* product title */}
      <div className="bg-primary p-4 p-md-7 d-flex align-items-center h-30 rounded-top-inside">
        <h3 className="text-white fs-h4 ms-4 ms-lg-0 letter-space-md-paragraph">{name}</h3>
      </div>
      {/* product info */}
      <div className="d-flex flex-column justify-content-between p-4 p-md-7">
        ...
        <ul className="d-flex justify-content-stretch flex-wrap">
          ...
          <li className="w-100p">
            <Btn data={name} getData={getServiceName} isOutline theme="primary" isDisabled={!descriptions}>{ isOpen ? '關閉' : '了解更多'}</Btn>
          </li>
        </ul>
      </div>
    </div>
